import { workerRoutes } from "@/lib/network-config";

export const TRAKT_API_BASE = "https://api.trakt.tv";
export const TRAKT_API_VERSION = "2";
export const TRAKT_CLIENT_ID = "gs_Y_y_FxpZ_aoqFc5hjMDCc2WcIcaE0XBwFQoiS2tg";
export const TRAKT_TOKEN_PROXY = workerRoutes.traktToken();
export const TRAKT_DEVICE_TOKEN_PROXY = workerRoutes.traktDeviceToken();
export const TRAKT_VERIFY_URL = "https://trakt.tv/activate";
export const REFRESH_THRESHOLD_SEC = 14 * 24 * 60 * 60;
export const WRITE_MIN_INTERVAL_MS = 1000;
