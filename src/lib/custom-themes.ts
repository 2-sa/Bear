...
const LAYOUTS = new Set<ThemeLayout>(["sidebar", "topdock", "rail", "stremio", "minui", "dracula", "nord", "forest", "royal", "cinematic", "custom"]);
...
export function parseThemeJson(text: string): { ok: true; theme: CustomTheme } | { ok: false; error: string } {
  ...
  if (!raw || typeof raw !== "object") {
    return { ok: false, error: "This file isn't a Bear theme." };
  }
