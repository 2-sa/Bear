...
export async function runWebhookTick(
  ...
  for (const rule of settings.webhookRules) {
    ...
    for (const channel of targets) {
      const url = channel === "discord" ? discordUrl! : telegramUrl!;
      const text = `Bear rule "${rule.name}": ${newMatched.length} new ${newMatched.length === 1 ? "release" : "releases"}`;
...
function headlineForSource(source: SourceKey, count: number): string {
  ...
  switch (source) {
    case "library":
      return `Bear: ${count} upcoming ${noun} from your library`;
    case "all":
      return `Bear: ${count} new upcoming ${noun}`;
    case "trakt":
      return `Bear: ${count} upcoming ${noun} from your Trakt watchlist`;
    case "anticipated":
      return `Bear: ${count} new entries on Trakt Anticipated`;
    case "custom":
      return `Bear: ${count} new ${noun} in your custom feed`;
  }
}
