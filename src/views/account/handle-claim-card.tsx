...
function HandleHeader({ hasCustom }: { hasCustom: boolean }) {
  ...
  return (
    <div className="flex flex-col gap-0.5">
      ...
      <span className="text-[12px] text-ink-subtle">
        {hasCustom ? t("How people find you across Bear.") : t("Claim one so people can find you across Bear.")}
      </span>
    </div>
  );
}
...
function StatusLine({ status, onPick }: { status: HandleStatus; onPick: (s: string) => void }) {
  ...
  if (status.state === "error")
    return <span className="text-[11.5px] text-ink-subtle">{t("Sign in to Bear to check availability.")}</span>;
