...
function autoPick(tracks: TrackInfo[], lang: string, primaryId: string | null): string | null {
  ...
  const pool = tracks.filter((track) => track.id !== primaryId);
...
export function useSecondarySub({
  ...
  useEffect(() => {
    ...
    if (!bridge) return;
    const primaryId = snap.subtitleTracks.find((track) => track.selected)?.id ?? null;
    const currentId = snap.subtitleTracks.find((track) => track.secondary)?.id ?? null;
