...
function buildActionableCastError(
  ...
  if (deviceKind === "roku" && /ROKU_ECP_BLOCKED|control by mobile apps|network access/i.test(err)) {
    return {
      ...
      message:
        "Your Roku is set to block control requests from apps on your network, so Bear can't reach it. This is a one-time setting on the Roku.",
      steps: [
        ...
        "Come back to Bear and try casting again.",
      ],
      deviceName,
    };
  }
  if (deviceKind === "roku" && /ROKU_ECP_NOT_FOUND/i.test(err)) {
    return {
      ...
      message:
        "Bear found something at this address that looked like a Roku, but it didn't respond like one. The device may be offline or another product picked up the same broadcast.",
      steps: [
  ...
  if (/ffmpeg/i.test(err)) {
    return {
      ...
      message:
        "Bear uses ffmpeg to convert streams into formats TVs can play. It's a one-time install and Bear will pick it up automatically.",
      steps: [
        ffmpegInstallStep(),
        "Restart Bear after the install completes.",
