...
function printDocs(root: HTMLElement) {
  ...
  doc.open();
  doc.write(
    `<!doctype html><html><head><meta charset="utf-8"><title>Bear Relay Documentation</title><style>${PRINT_CSS}</style></head><body><main>${root.innerHTML}</main></body></html>`,
  );
  doc.close();
...
function buildTxt(root: HTMLElement): string {
  ...
  return `Bear Relay Documentation\n${"=".repeat(28)}\n${lines.join("\n").trim()}\n`;
}
...
function buildJson(root: HTMLElement) {
  ...
  return {
    title: "Bear Relay Documentation",
