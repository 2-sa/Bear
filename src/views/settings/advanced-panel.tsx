  ...
  return (
    <>
      ...
      {supportsInAppUpdates && (
        <Section
          title={t("Updates")}
          subtitle={t("Bear checks our signed GitHub releases for new versions. Nothing installs until you choose to, and every installer must pass signature verification.")}
        >
      ...
      <Section
        ...
        subtitle={t("Export your entire Bear setup to a single file, then restore it on a new computer or keep it as a backup. Everything is included except your Stremio sign-in.")}
      >
        <SettingsRecoverRow />
        <BackupRow />
      </Section>
      ...
      <Section
        title={t("Downloads")}
        subtitle={t("Where Bear saves videos when you hit Download in the player. Pick any folder, including one on a different drive.")}
      >
        <DownloadsSection />
      </Section>
      ...
      <Section
        title={t("Privacy")}
        subtitle={t("Bear sends no telemetry. This also drops outbound ad, analytics, and tracker requests that addons or metadata providers try to make, before they leave your machine.")}
      >
        <PrivacyRow />
      </Section>
      ...
      {isTauri && (
        <Section
          title={t("System tray")}
          subtitle={t("Keep Bear a click away. Close it to the system tray instead of quitting, and control it from the tray menu. These also mirror into the tray menu live.")}
        >
          <TrayRow />
        </Section>
      )}
      ...
      {isTauri && (
        <Section
          ...
          subtitle={t("Bear catches stremio:// install links so the configure-and-install flow stays inside the app. Every install also syncs to your Stremio account, so the official app remains the canonical home for your library.")}
        >
          <StremioDeeplinkRow />
        </Section>
      )}
      ...
      {isTauri && (
        <Section
          ...
          subtitle={t("Let your Discord friends see what you are watching, with the show poster and a live progress bar. Desktop only, and only your own Discord client is involved (nothing touches a Bear server).")}
        >
          <DiscordPresenceRow />
        </Section>
      )}
  ...
  return (
    <section className="rounded-2xl border border-edge-soft bg-canvas/30 p-5">
      ...
      <p className="mt-2 text-[12px] leading-relaxed text-ink-muted">
        Bear is an independent, open-source desktop and web client. It is{" "}
    ...
    <div className="flex flex-col gap-2.5">
      <div className="flex items-start gap-3 rounded-xl border border-edge-soft bg-canvas/40 px-4 py-3.5">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <span className="text-[14px] font-medium text-ink">
            {t("Catch stremio:// install links inside Bear")}
          </span>
          <p className="text-[12.5px] leading-relaxed text-ink-subtle">
            {t("Bear's in-app installer animates the manifest install and keeps you in context. Anything Bear installs is also synced to your Stremio account, so the official app stays the canonical library. Turn this off and Stremio becomes the only handler for stremio:// links; Bear still installs anything you trigger from inside the app (Configure & install, paste, drag-and-drop).")}
          </p>
        </div>
        <button
          type="button"
      ...
      {on ? (
        <p className="px-1 text-[11.5px] leading-relaxed text-ink-subtle">
          {t("Heads up: if Stremio is also installed, Windows may ask which app to use the first time a stremio:// link fires. Pick Bear to make it stick.")}
        </p>
      ) : (
        <p className="px-1 text-[11.5px] leading-relaxed text-ink-subtle">
          {t("stremio:// links now open in the Stremio app. Bear will only install when you trigger it from inside Bear.")}
        </p>
      )}
    </div>
  );
    ...
    u.status === "checking"
      ...
      : u.status === "downloading"
        ...
        : u.status === "downloaded"
          ...
          : u.status === "installing"
            ? t("Installing. Bear will restart.")
            : u.status === "available"
              ...
              : u.status === "uptodate"
                ...
                : u.status === "error" && u.manualCheck
                  ...
                  : t("Bear checks automatically every few hours.");
  return (
    <div className="flex items-center gap-3 rounded-xl border border-edge-soft bg-canvas/40 px-4 py-3.5">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <span className="flex items-center gap-2 text-[14px] font-medium text-ink">
          {ready && u.version ? t("Bear {version} available", { version: u.version }) : `Bear ${__APP_VERSION__}`}
          <BetaTag />
        </span>
      ...
      {on && (
        <div className="flex flex-col gap-1.5 ps-4">
          ...
          <DiscordSubToggle
            ...
            hint={t("Display 'Browsing Bear' when nothing is playing.")}
