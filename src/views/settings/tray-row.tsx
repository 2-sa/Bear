    ...
    <div className="flex flex-col gap-2.5">
      <div className="flex items-start gap-3 rounded-xl border border-edge-soft bg-canvas/40 px-4 py-3.5">
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          ...
          <p className="text-[12.5px] leading-relaxed text-ink-subtle">
            {t("Closing the window tucks Bear into the tray instead of quitting, so it reopens instantly. Right-click the tray icon for quick controls, or pick Quit to exit fully.")}
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="flex flex-col gap-1.5 ps-4">
        <SubToggle
          ...
          hint={t("Keep the Bear window above other windows.")}
          on={settings.trayAlwaysOnTop}
          onToggle={() => update({ trayAlwaysOnTop: !settings.trayAlwaysOnTop })}
        />
        <SubToggle
          label={t("Pause when minimized")}
          hint={t("Stop playback when you minimize Bear or send it to the tray.")}
