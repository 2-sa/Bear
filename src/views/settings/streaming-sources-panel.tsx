  ...
  return (
    <>
      <Section
        ...
        subtitle={t("How aggressively Bear rejects shady or mismatched streams before showing them in the picker.")}
      >
        <StreamFilterPicker
      ...
      <Section
        ...
        subtitle={t("Bear ranking puts the best-scoring sources first. Addon order keeps each addon's results in the order it returned them, like the Stremio and Vidi apps. Stream priority below decides which addon leads, in both modes.")}
      >
        <StreamSortPicker
        ...
        <p className="mt-3 rounded-xl border border-edge-soft bg-canvas/40 px-4 py-3 text-[12.5px] leading-relaxed text-ink-muted">
          {t("Using AIOStreams or another aggregator addon? Its own sorting and filtering happen inside the addon before Bear ever sees the results, then Bear applies the stream filter and result order above on top. If results look thinner than expected, keep one side permissive: either relax the addon's internal filters or set Bear's stream filter to Balanced or Off.")}
        </p>
      </Section>
        ...
        <KeyField
          ...
          help={
            <>
              ...
              Used to check cache and unrestrict links. Bear never adds or removes torrents on
              its own.
            </>
          }
          headerExtra={
  ...
  const options: Array<{ id: "condensed" | "stremio"; label: string; sub: string }> = [
    {
      id: "condensed",
      label: t("Condensed"),
      sub: t("Default. Top pick at the top, quality tiles, and an All-Sources drawer. Bear scores and ranks results."),
    },
    {
      id: "stremio",
      label: "Stremio",
...
function StreamSortPicker({
  ...
  const options: Array<{ id: "harbor" | "addon"; label: string; sub: string }> = [
    {
      id: "harbor",
      label: t("Bear ranking"),
      sub: t("Default. Bear parses and scores every source and surfaces the best quality first."),
    },
    {
      id: "addon",
      label: t("Addon order"),
