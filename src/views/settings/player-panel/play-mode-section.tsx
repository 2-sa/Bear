...
export function PlayModePanel() {
  ...
  const choices: Array<{
  ...
  }> = [
    {
      id: "instant",
      label: t("Instant"),
      sub: t("Hitting Play jumps straight into playback with the best stream Bear finds."),
      recommended: true,
    },
    {
      id: "manual",
    ...
    },
    {
      ...
      sub: t("Pick a source once and Bear keeps playing the rest of that season from the same release, no re-picking. Works best with a debrid season pack. For anime it locks the whole series to that release."),
    },
  ];
    ...
    <div className="flex flex-col gap-2.5">
      ...
      >
        ...
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          ...
          <span className="text-[12.5px] leading-snug text-ink-muted">
            {t("When auto-playing the next episode, keep the same release/source you were just watching instead of Bear's top-ranked stream. Falls back to the best stream if that source isn't available.")}
          </span>
        </div>
      </button>
      <button
