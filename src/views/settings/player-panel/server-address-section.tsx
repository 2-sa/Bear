...
export function ServerAddressSection() {
  ...
  return (
    <Section
      ...
      subtitle={t(
        "Bear runs a small streaming server right on this computer. This is where it lives. To stream from this machine on another device, copy the Wi-Fi address and paste it into Remote streaming server in Bear over there.",
      )}
    >
          ...
          <div className="flex flex-col gap-1.5 rounded-md bg-elevated px-4 py-3.5">
            ...
            {PORT_TAKEN_RE.test(lastError) && (
              <span className="text-[12.5px] leading-relaxed text-ink-muted">
                {t(
                  "Another program already holds this port, usually a Stremio server that is running on this machine. Bear tried its spare ports too. Stop that server, or leave it running and point Bear at it in Remote streaming server below.",
                )}
              </span>
            )}
            {/not bundled/i.test(lastError) && (
              <span className="text-[12.5px] leading-relaxed text-ink-muted">
                {t(
                  "This usually means antivirus removed the server file (stremio-server.exe). Add Bear's install folder to your antivirus exclusions, then reinstall.",
                )}
              </span>
            )}
          </div>
      ...
      <p className="px-1 text-[12.5px] leading-relaxed text-ink-subtle">
        {t(
          "Looking for Bear in your browser, the phone remote, or the manga reader remote? They moved to the Remotes page.",
        )}
      </p>
    </Section>
  );
