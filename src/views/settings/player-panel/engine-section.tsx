...
export function PlayerEnginePanel() {
  ...
  const choices: Array<{
    ...
    },
    {
      id: "mpv",
      label: "mpv",
      sub: t("Bear's full video engine. Plays anything you throw at it."),
    },
  ];
    ...
    <DesktopOnlyBlock>
      <div className="flex flex-col gap-5">
        <SettingGroup label={t("Engine")}>
          ...
          {mpvProbe && !mpvProbe.available && (
            <div className="flex items-start gap-2.5 rounded-md bg-elevated px-4 py-3.5 text-[12.5px] leading-relaxed text-ink">
              ...
              <span className="flex min-w-0 flex-1 flex-col gap-2">
                <span>
                  {t(
                    "libmpv did not load, so playback falls back to HTML5 and formats like MKV may refuse to play. On Linux, install your distribution's libmpv package, then restart Bear.",
                  )}
                </span>
                {mpvProbe.error && (
          ...
          <ToggleRow
            label={t("Embed mpv inside Bear window")}
            sub={t("Renders mpv inline so playback lives in Bear itself. Disable to open it in a separate window instead.")}
