  ...
  return (
    <>
      ...
      <Section
        ...
        subtitle={t(
          "When playback starts, Bear automatically finds and loads a subtitle in one of these languages, so you never have to search by hand. The first available match wins, so put your main language first.",
        )}
      >
        <LanguagesPicker
        ...
        <ToggleRow
          ...
          sub={t(
            "Bear still finds and loads subtitles so they're one click away in the player, it just won't turn them on automatically.",
          )}
        ...
        <ToggleRow
          ...
          sub={t(
            "Downloaded subtitles can arrive after playback starts. Bear switches from a temporary track when a stronger release match becomes available.",
          )}
        ...
        <ToggleRow
          ...
          sub={t(
            "After you pick a source, show a subtitle picker so you can set the exact track and language before the video starts. Off by default, Bear keeps picking one for you automatically.",
          )}
      ...
      <Section
        ...
        subtitle={t(
          "When a release ships multiple audio tracks, Bear selects the first match from this list.",
        )}
      >
        <LanguagesPicker
      ...
      >
        ...
        <div className="mt-2 flex flex-col gap-3 rounded-xl border border-edge-soft bg-canvas/30 p-4 sm:flex-row sm:items-center sm:justify-between">
          <p className="text-[13px] leading-relaxed text-ink-muted sm:max-w-[480px]">
            {t(
              "Heads up: Bear was built in English. Multi-language support is partial, so your addons usually catch what Bear's own filters miss. If you speak another language and want to help fill the gaps, the source is open.",
            )}
          </p>
          <button
