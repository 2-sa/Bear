...
Bear is built as `dev.twosa.bear.beta` against the pinned GNOME 49 SDK. mpv is
...
```sh
flatpak-builder --user --force-clean --state-dir=.flatpak-work/state --repo=.flatpak-work/repo .flatpak-work/build flatpak/dev.twosa.bear.beta.yml
flatpak build-bundle .flatpak-work/repo Bear.flatpak dev.twosa.bear.beta
flatpak install --user Bear.flatpak
```
...
To audit the sandbox, run `flatpak run --command=sh dev.twosa.bear.beta` and verify
...
After updating Bear's version and lockfiles, refresh all Flatpak release
