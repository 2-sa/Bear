...
test("season downloads resolve serially so local P2P setup cannot overlap", () => {
  assert.match(seasonDownload, /const limit = limiter\(1\)/);
  assert.match(seasonDownload, /resolveBestDownload\([\s\S]*allowP2p: true/);
});
