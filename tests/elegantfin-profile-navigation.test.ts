...
test("Bear bridge exposes the immediate profile helper", () => {
...
test("Theme Studio documents the immediate boolean profile API", () => {
  assert.deepEqual(
    WINDOW_HARBOR.find((entry) => entry.call === "window.harbor.tryViewMyProfile()"),
    {
      ...
      desc: "Open the signed-in user's own Bear profile immediately when its handle is loaded. Returns true when navigation starts and false otherwise.",
    },
  );
});
