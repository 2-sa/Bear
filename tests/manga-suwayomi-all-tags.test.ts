...
before(async () => {
  httpServer = createHttpServer((req, res) => {
    ...
    if (req.url === "/api/v1/source/11/popular/1") {
      res.end(
        JSON.stringify({ mangaList: [{ id: 101, title: "First manga" }], hasNextPage: false }),
      );
      return;
    }
    if (req.url === "/api/v1/source/22/popular/1") {
      res.end(
        JSON.stringify({ mangaList: [{ id: 202, title: "Second manga" }], hasNextPage: false }),
      );
      return;
    }
    if (req.url === "/api/v1/source/33/popular/1") return;
  ...
  if (!address || typeof address === "string") throw new Error("Suwayomi test server failed");
...
test("All tags lists popular manga from every Suwayomi source by default", async () => {
  ...
  assert.deepEqual(
    manga.map((item: { title: string }) => item.title).sort(),
