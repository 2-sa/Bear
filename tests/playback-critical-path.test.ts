...
test("startup buffering is staged and adapts without lowering media quality", () => {
  ...
  assert.match(mpv, /expectedLoadId !== mediaLoadId/);
  assert.match(mpv, /bufferPolicyQueue = bufferPolicyQueue/);
  assert.match(mpv, /applyDefaultVodBufferPhase\(profile, "steady", mediaLoadId\)/);
