...
test("viewer uses Bear identity and the fixed remote-content boundary", () => {
...
test("viewer uses truthful slow-loading copy", () => {
  ...
  assert.match(source, /may not support Bear(?:'|&apos;)s temporary viewer/);
...
test("warning uses a direct main action and an alternate-only menu", () => {
  ...
  assert.match(splitSource, /Continue in Bear/);
